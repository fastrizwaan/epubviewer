...
# =============================================================================
# LibraryManager - Self-contained library management
# =============================================================================
class LibraryManager:
    """Self-contained class for managing EPUB library data and persistence."""
    
    # Class-level constants
    COVER_W, COVER_H = 70, 100  # sidebar cover size
    LIB_COVER_W, LIB_COVER_H = 200, 300  # library view cover size
    
    def __init__(self, library_dir=None):
        """Initialize the library manager.
        
        Args:
            library_dir: Custom library directory. If None, uses default user data dir.
        """
        ...
        self.library_data = self.load()
    
    def _ensure_directories(self):
        """Create necessary directories if they don't exist."""
        os.makedirs(self.library_dir, exist_ok=True)
        os.makedirs(self.covers_dir, exist_ok=True)
        os.makedirs(self.book_settings_dir, exist_ok=True)
    ...
    def load(self):
        ...
        if os.path.exists(self.library_file):
            try:
                with open(self.library_file, "r", encoding="utf-8") as fh:
                    return json.load(fh)
            except Exception as e:
    ...
    def save(self, data=None):
        ...
        Args:
            data: Library data to save. If None, uses self.library_data
        """
        if data is not None:
            self.library_data = data
        
        try:
            with open(self.library_file, "w", encoding="utf-8") as fh:
                json.dump(self.library_data, fh, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"Error saving library: {e}")
    
    def add_book(self, book_path, title, author, cover_image=None):
        ...
        Args:
            ...
            title: Book title
            author: Book author
            cover_image: PIL Image or GdkPixbuf for cover
            
        Returns:
            dict: The library entry created
        """
        entry = {
            "path": book_path,
            "title": title,
        ...
        if cover_image:
            cover_path = self.save_cover(book_path, cover_image)
            if cover_path:
                entry["cover"] = cover_path
        
        # Remove existing entry if present
        self.library_data = [e for e in self.library_data if e.get("path") != book_path]
        self.library_data.append(entry)
        self.save()
        
        return entry
    
    def remove_book(self, book_path):
        """Remove a book from the library.
        
        Args:
            book_path: Path to the EPUB file to remove
        """
        self.library_data = [e for e in self.library_data if e.get("path") != book_path]
        self.save()
        ...
        # Remove cover file
        try:
            cover_hash = hashlib.sha1(book_path.encode("utf-8")).hexdigest()[:16]
            cover_path = os.path.join(self.covers_dir, f"{cover_hash}.png")
            if os.path.exists(cover_path):
                os.remove(cover_path)
        except Exception:
            pass
    
    def update_progress(self, book_path, index, progress):
        """Update reading progress for a book.
        
        Args:
            book_path: Path to the EPUB file
            index: Current chapter/item index
            progress: Progress as float (0.0 to 1.0)
        """
        for entry in self.library_data:
            if entry.get("path") == book_path:
                entry["index"] = int(index)
                entry["progress"] = float(progress)
                self.save()
                break
    
    def get_book_entry(self, book_path):
        """Get library entry for a specific book.
        
        Args:
            book_path: Path to the EPUB file
            
        Returns:
            dict or None: Library entry if found
        """
        for entry in self.library_data:
            if entry.get("path") == book_path:
                return entry
    ...
    def save_cover(self, book_path, image, for_library=True):
        """Save a cover image.
        
        Args:
            book_path: Path to the EPUB file (used for filename)
            image: PIL Image or GdkPixbuf
            for_library: If True, use library size; else use sidebar size
            
        Returns:
            str: Path to saved cover file
        """
        try:
            cover_hash = hashlib.sha1(book_path.encode("utf-8")).hexdigest()[:16]
            cover_path = os.path.join(self.covers_dir, f"{cover_hash}.png")
            
            target_w = self.LIB_COVER_W if for_library else self.COVER_W
            target_h = self.LIB_COVER_H if for_library else self.COVER_H
            
            # Handle GdkPixbuf
            if isinstance(image, GdkPixbuf.Pixbuf):
                scaled = image.scale_simple(target_w, target_h, GdkPixbuf.InterpType.BILINEAR)
                scaled.savev(cover_path, "png", [], [])
            else:
                # Assume PIL Image
                image.thumbnail((target_w, target_h), resample=1)
    ...
    def get_cover_path(self, book_path):
        ...
        Returns:
            str or None: Path to cover file if it exists
        """
        entry = self.get_book_entry(book_path)
    ...
    def get_book_settings_file(self, book_path):
        """Get the settings file path for a specific book.
        ...
        Returns:
            str or None: Path to settings file
        """
        if not book_path:
            return None
        book_hash = hashlib.sha1(book_path.encode("utf-8")).hexdigest()[:16]
        return os.path.join(self.book_settings_dir, f"{book_hash}.json")
    
    def save_book_settings(self, book_path, settings):
        """Save settings for a specific book.
        ...
        """
        settings_file = self.get_book_settings_file(book_path)
        if not settings_file:
            return
        try:
            with open(settings_file, "w", encoding="utf-8") as fh:
    ...
    def load_book_settings(self, book_path):
        """Load settings for a specific book.
        
        Args:
            book_path: Path to the EPUB file
            
        Returns:
            dict or None: Settings if found
        """
        settings_file = self.get_book_settings_file(book_path)
        if not settings_file or not os.path.exists(settings_file):
            return None
...
# =============================================================================
# EPubRenderer - Self-contained EPUB parsing and rendering
# =============================================================================
class EPubRenderer:
    """Self-contained class for EPUB parsing, content extraction, and HTML rendering."""
    ...
    def load_epub(self, epub_path):
        ...
        try:
            ...
            # Build item and href maps
            self.item_map = {}
            self.href_map = {}
            for idx, item in enumerate(self.items):
                self.item_map[item.get_name()] = idx
                href = item.get_name()
                if "#" in href:
                    href = href.split("#")[0]
                self.href_map[href] = idx
    ...
    def _extract_metadata(self):
        ...
        Returns:
            dict: Metadata including title, author, etc.
        """
        metadata = {
            "title": "Unknown",
            "author": "Unknown",
        ...
        try:
            pub = self.book.get_metadata('DC', 'publisher')
            if pub and pub[0]:
        ...
        try:
            desc = self.book.get_metadata('DC', 'description')
            if desc and desc[0]:
                metadata["description"] = desc[0][0]
        except Exception:
            pass
    ...
    def _parse_toc(self):
        ...
        Returns:
            list: TOC structure as TocItem objects
        """
        if not self.book:
            return []
    ...
    def _build_toc_tree(self, toc_items, parent=None):
        """Recursively build TOC tree.
        
        Args:
            toc_items: TOC items from ebooklib
            parent: Parent TocItem (for nested items)
            
        Returns:
            list: List of TocItem objects
        """
        result = []
        ...
        for item in toc_items:
            if isinstance(item, tuple):
                ...
                title = getattr(section, 'title', 'Chapter')
                href = getattr(section, 'href', '')
                
                # Find index
                index = self._find_index_by_href(href)
                
                # Build children
                child_items = self._build_toc_tree(children)
                
                toc_item = TocItem(title=title, href=href, index=index, children=child_items)
                ...
                toc_item = TocItem(title=title, href=href, index=index)
                result.append(toc_item)
        
        return result
    
    def _find_index_by_href(self, href):
        """Find item index by href.
        
        Args:
            href: The href to look up
            
        Returns:
            int: Item index or -1 if not found
        """
        if not href:
            return -1
        ...
        return self.href_map.get(clean_href, -1)
    ...
    def clean_html(self, html_content, allowed_tags=None, allowed_attrs=None,
        ...
        Args:
            ...
            remove_processing_instructions: Remove PI nodes
        ...
        # Quick removals by regex (pre-parse)
        if remove_processing_instructions:
            html_content = re.sub(r'<\?[^>]*\?>', '', html_content, flags=re.IGNORECASE)
        html_content = re.sub(r'dp\s*n="[^"]*"\s*folio="[^"]*"\s*\?*', '', html_content, flags=re.IGNORECASE)
        html_content = re.sub(r'(?m)^[\s\?]{1,8}\n?', '', html_content)
        html_content = html_content.replace('??', '')
        
        # Parse for structured cleaning
        try:
            soup = BeautifulSoup(html_content, "lxml")
        except Exception:
            soup = BeautifulSoup(html_content, "html.parser")
        
        # Remove ProcessingInstruction nodes
        ...
        # Fully remove script-like and embed tags
        ...
        # Remove tiny punctuation-only nodes and normalize whitespace
        for t in list(soup.find_all(string=True)):
            s = str(t)
            # Remove nodes that are purely question marks / whitespace
    ...
    def extract_text(self, index):
        ...
        """
        html = self.get_item_content(index)
        if not html:
            return ""
    ...
    def get_cover_image(self):
        ...
        Returns:
            GdkPixbuf.Pixbuf or None: Cover image
        """
        if not self.book:
            return None
    ...
    def extract_sentences(self, index):
        """Extract sentences from an item for TTS.
        
        Args:
            index: Item index
            
        Returns:
            list: List of dicts with 'sid' and 'text' keys
        """
        html = self.get_item_content(index)
        if not html:
            return []
        ...
        try:
            ...
            # Normalize drop caps
            self._normalize_dropcaps(soup)
            
            # Block-level tags that create sentence boundaries
            ...
            def extract_from_element(element):
                ...
                if element.name in block_tags:
                    ...
                    if block_text:
                        block_sentences = self._split_into_sentences(block_text)
                        sentences.extend(block_sentences)
                    
                    # Recurse into nested block elements
                    for child in element.children:
                        if isinstance(child, Tag) and child.name in block_tags:
                            extract_from_element(child)
                else:
                    # Non-block: traverse children
                    for child in element.children:
                        if isinstance(child, Tag):
                            ...
                            txt = str(child).strip()
                            if txt:
                                block_sentences = self._split_into_sentences(txt)
                                sentences.extend(block_sentences)
            
            body = soup.find("body") or soup
            extract_from_element(body)
            
            # Create output with sentence IDs
            output = []
            for sid, sentence in enumerate(sentences):
                s = sentence.strip()
                if s:
                    output.append({"sid": sid, "text": s})
    ...
    def _normalize_dropcaps(self, soup):
        """Normalize drop cap elements in HTML.
    ...
    def search_text(self, query, case_sensitive=False):
        """Search for text across all items.
        ...
        Returns:
            list: List of dicts with 'index', 'text', and 'context' keys
        """
        results = []
...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # library
        self.library = self.library_manager.library_data
        ...
        self.ann_listbox = Gtk.ListBox()
        self.ann_listbox.set_selection_mode(Gtk.SelectionMode.NONE)
        self.ann_listbox.connect("row-activated", self._on_annotation_activated)
        self.ann_listbox.append(Gtk.Label(label="No annotations"))
        ann_scrolled = Gtk.ScrolledWindow(); ann_scrolled.set_child(self.ann_listbox)
        self.side_stack.add_titled(ann_scrolled, "annotations", "Annotations")

        # Bookmarks list
        self.bm_listbox = Gtk.ListBox()
        self.bm_listbox.set_selection_mode(Gtk.SelectionMode.NONE)
        self.bm_listbox.connect("row-activated", self._on_bookmark_activated)
        bm_scrolled = Gtk.ScrolledWindow()
        bm_scrolled.set_vexpand(True)
        bm_scrolled.set_child(self.bm_listbox)
        
        # Add "Add Bookmark" button at top
        bm_container = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        add_bm_btn = Gtk.Button(label="📑 Add Bookmark")
        add_bm_btn.connect("clicked", self._add_manual_bookmark)
        add_bm_btn.set_margin_top(6)
        add_bm_btn.set_margin_start(6)
        add_bm_btn.set_margin_end(6)
        bm_container.append(add_bm_btn)
        bm_container.append(bm_scrolled)
        
        self.side_stack.add_titled(bm_container, "bookmarks", "Bookmarks")
        ...
        # --- Dictionary tab ---
        dict_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
        dict_box.set_margin_top(6); dict_box.set_margin_bottom(6)
        dict_box.set_margin_start(6); dict_box.set_margin_end(6)
        ...
        self.dict_scrolled.set_vexpand(True)
        dict_box.append(self.dict_scrolled)
        
        # Initialize as None - will be created after main webview
        self.dict_webview = None
        ...
        self.search_info_label.set_xalign(0)
        self.search_info_label.add_css_class("dim-label")
        self.search_info_label.set_margin_top(6)
        self.search_info_label.set_margin_bottom(6)
        search_tab_box.append(self.search_info_label)
        
        # Search results list
        self.search_listbox = Gtk.ListBox()
        self.search_listbox.set_selection_mode(Gtk.SelectionMode.NONE)
        self.search_listbox.connect("row-activated", self._on_search_result_activated)
        search_scrolled = Gtk.ScrolledWindow()
        search_scrolled.set_vexpand(True)
        search_scrolled.set_child(self.search_listbox)
        search_tab_box.append(search_scrolled)
        
        self.side_stack.add_titled(search_tab_box, "search", "Search")
        ...
        self.tab_dict = make_icon_tab("accessories-dictionary-symbolic", "Dictionary", "dictionary")
        self.tab_search = make_icon_tab("system-search-symbolic", "Search", "search")
        self.tab_font = make_icon_tab("format-text-rich-symbolic", "Font", "font")
        self.tab_toc.set_active(True)
        tabs_box.append(self.tab_toc); tabs_box.append(self.tab_ann); tabs_box.append(self.tab_bm); tabs_box.append(self.tab_read); tabs_box.append(self.tab_dict); tabs_box.append(self.tab_search); tabs_box.append(self.tab_font)
        ...
        try:
            ...
            # Track vertical scroll for single-column progress
            vadj = self.scrolled.get_vadjustment()
            self._vadj_scroll_timer = None
            def _on_vadj_changed(adj):
                # Use a non-starving throttle: if a timer is already pending, don't reset it.
                # This ensures we get updates even during high-frequency scrolling.
                if self._vadj_scroll_timer is None:
                    # print(f"[SCROLL-GTK] ⏱ Scheduling update for {adj.get_value():.1f}")
                    from gi.repository import GLib
                    self._vadj_scroll_timer = GLib.timeout_add(20, lambda: self._on_vertical_scroll(adj))
            vadj.connect("value-changed", _on_vadj_changed)
            # Track horizontal scroll for multi-column progress
            hadj = self.scrolled.get_hadjustment()
            self._hadj_scroll_timer = None
            def _on_hadj_changed(adj):
                # Use a non-starving throttle: if a timer is already pending, don't reset it.
                # This ensures we get updates even during high-frequency scrolling.
                if self._hadj_scroll_timer is None:
                    # print(f"[SCROLL-GTK] ⏱ Scheduling horizontal update for {adj.get_value():.1f}")
                    from gi.repository import GLib
                    self._hadj_scroll_timer = GLib.timeout_add(20, lambda: self._on_horizontal_scroll(adj))
            hadj.connect("value-changed", _on_hadj_changed)
            try: 
            ...
            # ADD SCROLL POSITION MESSAGE HANDLER
            try:
                content_manager = self.webview.get_user_content_manager()
                content_manager.connect("script-message-received::scrollPosition", 
                                        self._on_scroll_position_received)
                content_manager.register_script_message_handler("scrollPosition")
                print("✓ Scroll position handler registered")
            except Exception as e:
                print(f"⚠ Could not register scroll position handler: {e}")
            
            # ADD ANNOTATION REQUEST HANDLER
            try:
                content_manager = self.webview.get_user_content_manager()
                content_manager.connect("script-message-received::annotationRequest", 
                                        self._on_annotation_request)
                content_manager.register_script_message_handler("annotationRequest")
                print("✓ Annotation request handler registered")
            except Exception as e:
                print(f"⚠ Could not register annotation handler: {e}")
            
            # ADD DEFINE REQUEST HANDLER
            try:
                content_manager = self.webview.get_user_content_manager()
                content_manager.connect("script-message-received::defineRequest", 
                                        self._on_define_request)
                content_manager.register_script_message_handler("defineRequest")
                print("✓ Define request handler registered")
            except Exception as e:
                print(f"⚠ Could not register define handler: {e}")
            
            # ADD SPEAK REQUEST HANDLER
            try:
                content_manager = self.webview.get_user_content_manager()
                content_manager.connect("script-message-received::speakRequest", 
                                        self._on_speak_request)
                content_manager.register_script_message_handler("speakRequest")
                print("✓ Speak request handler registered")
            except Exception as e:
                print(f"⚠ Could not register speak handler: {e}")
            
            # ADD BOOKMARK REQUEST HANDLER
            try:
                content_manager = self.webview.get_user_content_manager()
                content_manager.connect("script-message-received::bookmarkRequest", 
                                        self._on_bookmark_request)
                content_manager.register_script_message_handler("bookmarkRequest")
                print("✓ Bookmark request handler registered")
            except Exception as e:
                print(f"⚠ Could not register bookmark handler: {e}")
            
            # INITIALIZE DICTIONARY WEBVIEW (now that WebKit is available)
            try:
                ...
                def _on_load_changed(webview, load_event):
                    ...
                    try:
                        ...
                        if finished:
                            ...
                            print("[SCROLL] 🔄 Page loaded, scheduling restore")
                            GLib.timeout_add(400, lambda: (self._restore_scroll_simple(), False)[1])
                            
                            # Re-apply annotation highlights for current chapter
                            print("[ANNOTATION] 🔄 Re-applying highlights for current chapter")
    ...
    def _on_horizontal_scroll(self, adj):
        """Handle horizontal scroll from GTK ScrolledWindow (multi-column mode)."""
        # Always reset the timer first so the throttle can accept new events
        self._hadj_scroll_timer = None
...
###################
    ...
    def _stop_reading(self, path=None):
        try:
            ...
            # Auto-save bookmark before closing
            try:
                if self.current_index is not None and self.book:
    ...
    def _wrap_html(self, raw_html, base_uri):
            ...
            (function() {{
                ...
                if (container) {{
                    ...
                    const onScroll = function() {{
                        ...
                        if (!progressTimer) {{
                            ...
                            progressTimer = setTimeout(() => {{ progressTimer = null; }}, 30); // 30ms throttle
                        }}
                        
                        // 2. Delayed snap (only for multi-column)
                        if (!window.isSingleColumnMode) {{
                            clearTimeout(snapTimer);
                            snapTimer = setTimeout(() => {{
                                snapScroll();
                                sendScrollPositionToPython(); // Final update after snap
                            }}, 300); // Wait for scroll to stop before snapping
                        }}
                    }};
                ...
                window.addEventListener('wheel', function(e) {{
                    ...
                    if (targetCol !== currentCol) {{
                        ...
                        setTimeout(sendScrollPositionToPython, 400);
                    }}
        ...
        (function(){
            ...
            function createContextMenu(hasSelection) {
                if (contextMenu) {
            ...
            document.addEventListener('contextmenu', function(e) {
                ...
                if (selectedText && selectedText.length > 0) {
                    ...
                    const wordsBefore = fullText.substring(0, startIndex).trim().split(/\\s+/).slice(-20).join(' ');
                    const wordsAfter = fullText.substring(startIndex + selectedText.length).trim().split(/\\s+/).slice(0, 20).join(' ');
                    
                    // Get DOM path
                    let elem = container.nodeType === Node.TEXT_NODE ? container.parentElement : container;
                    const path = [];
                    ...
                    // Show context menu
                    const menu = createContextMenu(true);
                    ...
                    // Show context menu
                    const menu = createContextMenu(false);
                    menu.style.left = e.pageX + 'px';
                    menu.style.top = e.pageY + 'px';
                    menu.style.display = 'block';
                    
                    // Handle Speak from here button
                    menu.querySelector('.context-menu-speak').onclick = function() {
                        menu.style.display = 'none';
                        
                        if (window.webkit && window.webkit.messageHandlers && 
                            window.webkit.messageHandlers.speakRequest) {
                            window.webkit.messageHandlers.speakRequest.postMessage(JSON.stringify({
                                textContent: textFromHere
                            }));
                        }
                    };
        ...
        head = (
            ...
            link_intercept_script + js_detect_columns + annotation_menu_script
        )
        ...
        try:
          ...
          # set title and app name
            ...
            if resume:
                ...
                print(f"[SCROLL] 📂 Loading saved positions")
                self.saved_scroll_positions = {}
                scroll_data = None
                
                if loaded_settings and "scroll_positions" in loaded_settings:
                    scroll_data = loaded_settings.get("scroll_positions", {})
                else:
                    for e in self.library:
                        if e.get("path") == path:
                            scroll_data = e.get("scroll_positions", {})
                            break
                
                if scroll_data:
                    # Handle both old list format [left, top] and new dict format
                    for k, v in scroll_data.items():
                        try:
                            chapter_idx = int(k)
                            if isinstance(v, dict):
                                # New format: already a dict
                                self.saved_scroll_positions[chapter_idx] = v
                            elif isinstance(v, list) and len(v) >= 2:
                                # Old format: convert list to dict
                                self.saved_scroll_positions[chapter_idx] = {
                                    'mode': 'multi' if v[0] > 0 else 'single',
                                    'scrollLeft': int(v[0]),
                                    'scrollTop': int(v[1]),
                                    'columnIndex': None,
                                    'percentage': 0.0
    ...
    def update_navigation(self):
        ...
        if total > 0:
            intra_pct = 0.0
            if hasattr(self, 'saved_scroll_positions') and self.current_index in self.saved_scroll_positions:
                pos_data = self.saved_scroll_positions[self.current_index]
                if isinstance(pos_data, dict):
                    intra_pct = pos_data.get('percentage', 0.0)
    ...
    def _on_slider_changed(self, scale):
        ...
        if target_chapter != self.current_index:
            self.current_index = target_chapter
            self.update_navigation()
            self.display_page()
        
        # Handle intra-chapter scroll
        # 1. Try GTK vertical adjustment first (for single-column mode)
        vadj = self.scrolled.get_vadjustment()
        max_vscroll = vadj.get_upper() - vadj.get_page_size()
        
        if max_vscroll > 5:
            self._slider_updating = True
            vadj.set_value(intra_pct * max_vscroll)
            self._slider_updating = False
        # 2. Otherwise use JavaScript (for multi-column mode)
        elif intra_pct > 0.001:
            js = f"""
            (function() {{
                if (window.isSingleColumnMode) {{
                    var container = document.querySelector('.ebook-content');
                    if (container) {{
                        ...
                        if (maxScroll <= 0) {{
                            maxScroll = Math.max(0, (document.documentElement.scrollHeight || document.body.scrollHeight) - window.innerHeight);
                            window.scrollTo(0, Math.round({intra_pct} * maxScroll));
                        }} else {{
                            container.scrollTop = Math.round({intra_pct} * maxScroll);
                        }}
                    }}
                }} else {{
                    var metrics = typeof getContainerMetrics === 'function' ? getContainerMetrics() : null;
                    if (metrics && metrics.maxCol > 0) {{
                        var targetCol = Math.round({intra_pct} * metrics.maxCol);
                        if (typeof scrollToColumnIndex === 'function') {{
                            scrollToColumnIndex(targetCol, false);
                        }} else {{
                            metrics.container.scrollLeft = targetCol * metrics.pageWidth;
                        }}
                    }}
                }}
            }})();
            ...
            try:
                from gi.repository import GLib
                GLib.timeout_add(350, lambda: self._eval_js(js))
            except Exception:
                pass

    def _eval_js(self, js):
        """Helper to evaluate JS in the webview."""
        if not self.webview:
            return False
        try:
            self.webview.evaluate_javascript(js, -1, None, None, None, None, None)
    ...
    def _add_chapter_marks(self):
        """Add chapter boundary marks to the navigation slider."""
        self.nav_slider.clear_marks()
        total = len(self.items) if hasattr(self, 'items') and self.items else 0
        if total <= 1:
            return
...
, "r", encoding="utf-8", errors="ignore") as fh:
    ...
    def _capture_scroll_simple(self):
        """Simple scroll capture with extensive logging."""
        if not getattr(self, "webview", None) or self.current_index is None:
            print("[SCROLL] ❌ No webview or index")
            return
        
        print(f"[SCROLL] 📸 Capturing scroll for chapter {self.current_index}")
        
        # SUPER SIMPLE: just get both scroll values and log everything
        js_code = """
        (function() {
            ...
            if (container) {
                ...
                console.log('Returning:', left + '|' + top);
                return left + '|' + top;
            }
            
            console.log('No container, returning 0|0');
            return '0|0';
        })();
        """
        ...
        def on_result(webview, result, user_data):
            try:
                print(f"[SCROLL] 📥 Got result from JS")
                ...
                print(f"[SCROLL] ❌ Error in callback: {e}")
                import traceback
                traceback.print_exc()
        
        try:
            self.webview.evaluate_javascript(js_code, -1, None, None, None, on_result, None)
            print(f"[SCROLL] ✓ evaluate_javascript called")
        except Exception as e1:
            print(f"[SCROLL] ⚠️ evaluate_javascript failed: {e1}")
            try:
                self.webview.run_javascript(js_code, None, on_result, None)
                print(f"[SCROLL] ✓ run_javascript called (fallback)")
            except Exception as e2:
                print(f"[SCROLL] ❌ Both JS methods failed: {e2}")
    
    def _capture_scroll_with_callback(self, done_callback):
        """Capture scroll and call callback when done."""
        if not getattr(self, "webview", None) or self.current_index is None:
            print("[SCROLL] ❌ No webview or index")
            done_callback()
            return
        
        print(f"[SCROLL] 📸 Capturing scroll for chapter {self.current_index} (with callback)")
        
        js_code = """
        (function() {
            console.log('=== CAPTURE SCROLL (WITH CALLBACK) ===');
            var container = document.querySelector('.ebook-content');
            console.log('Container found:', container ? 'YES' : 'NO');
    ...
    def _restore_scroll_simple(self):
        """Intelligent scroll restore that handles layout mode changes."""
        if not getattr(self, "webview", None) or self.current_index is None:
            print("[SCROLL] ❌ No webview or index for restore")
            return
        
        saved = self.saved_scroll_positions.get(self.current_index)
        if not saved:
            print(f"[SCROLL] ℹ️ No saved position for chapter {self.current_index}")
            return
        
        # Handle old format (tuple) for backward compatibility
        if isinstance(saved, tuple):
            left, top = saved
            saved_mode = 'multi' if left > 0 else 'single'
            col_index = None
            percentage = 0.0
            print(f"[SCROLL] 📖 Old format detected, converting: left={left}, top={top}")
        else:
            # New format (dict)
            saved_mode = saved.get('mode', 'multi')
            left = saved.get('scrollLeft', 0)
            top = saved.get('scrollTop', 0)
        ...
        try:
            self.webview.evaluate_javascript(js_code, -1, None, None, None, None, None)
            print(f"[SCROLL] ✓ Restore JS executed")
        except Exception as e1:
            print(f"[SCROLL] ⚠️ evaluate_javascript failed: {e1}")
            try:
    ...
    def _get_chapter_title(self, chapter_index):
        """Get title for a chapter index."""
        try:
            if chapter_index < 0 or chapter_index >= len(self.items):
                return f"Chapter {chapter_index + 1}"
            
            # Try to find in TOC
            for href, toc_item in self.href_map.items():
                if hasattr(toc_item, 'index') and toc_item.index == chapter_index:
                    title = getattr(toc_item, 'title', None)
                    if title:
                        return title
            
            # Fallback to item name
            item = self.items[chapter_index]
    ...
    def _add_manual_bookmark(self, *args):
        """Add a manual bookmark at current position with text context."""
        print(f"[BOOKMARK] 📑 _add_manual_bookmark called")
        
        if self.current_index is None or not self.book:
            print(f"[BOOKMARK] ⚠️ Cannot add manual bookmark: no book loaded")
            return
        
        # Get text context from current position
        text_context = ""
        dom_path = ""
        element_index = 0
        
        if self.webview:
            # JavaScript to get text context at current position
            js = """
            (function() {
                // Get current position in viewport
                const container = document.querySelector('.ebook-content');
                if (!container) return JSON.stringify({error: 'No content'});
                ...
                if (!elem) return JSON.stringify({error: 'No element found'});
                
                // Get parent paragraph or div
                let contextElem = elem.closest('p, div, section, article') || elem;
                
                // Get text context (at least 30 words)
                let textContent = contextElem.textContent || '';
                let words = textContent.trim().split(/\\s+/);
                if (words.length < 30) {
                    // Try to get more context from siblings
                    let sibling = contextElem.nextElementSibling;
                    while (sibling && words.length < 50) {
                        let siblingText = sibling.textContent || '';
                        words = words.concat(siblingText.trim().split(/\\s+/));
                        sibling = sibling.nextElementSibling;
                    }
                }
                
                // Limit to 50 words for storage
                ...
                // Get element index in document
                const allElems = container.querySelectorAll('p, div, section, article');
                let elemIndex = Array.from(allElems).indexOf(contextElem);
            ...
            try:
                # Execute JavaScript to get context
                result = self.webview.evaluate_javascript_finish(
                ...
                if result:
                    ...
                    element_index = data.get('index', 0)
            except Exception as e:
                print(f"[BOOKMARK] Error getting text context: {e}")
        
        if not text_context:
            # Fallback: use first 50 words of chapter
        ...
        print(f"[BOOKMARK] 📑 Added manual bookmark: {chapter_title}")
    ...
    def _remove_bookmark(self, bookmark_index):
        ...
        print(f"[BOOKMARK] 🗑️ _remove_bookmark called for index {bookmark_index}")
        
        if 0 <= bookmark_index < len(self.bookmarks):
            removed = self.bookmarks.pop(bookmark_index)
            print(f"[BOOKMARK] 🗑️ Removed bookmark: {removed['chapter_title']}")
            print(f"[BOOKMARK]    Total bookmarks now: {len(self.bookmarks)}")
            
            # Update UI directly
            try:
                self._update_bookmarks_list()
                print(f"[BOOKMARK] ✓ UI updated after removal")
            except Exception as e:
                print(f"[BOOKMARK] ⚠️ UI update failed: {e}")
    ...
    def _update_bookmarks_list(self):
        ...
        if not self.bookmarks:
            ...
            self.bm_listbox.append(label)
            print(f"[BOOKMARK] 📋 Showing empty state")
            return
        
        # Sort by timestamp (newest first)
        sorted_bookmarks = sorted(self.bookmarks, key=lambda b: b['timestamp'], reverse=True)
        print(f"[BOOKMARK] 📋 Displaying {len(sorted_bookmarks)} bookmarks")
        
        for i, bookmark in enumerate(sorted_bookmarks):
            row = Gtk.ListBoxRow()
            row.bookmark_data = bookmark
            row.bookmark_index = self.bookmarks.index(bookmark)  # Original index
            ...
            # Icon
            icon_name = "user-bookmarks-symbolic" if bookmark['type'] == 'manual' else "document-save-symbolic"
            ...
            title_label = Gtk.Label(label=bookmark['chapter_title'])
            ...
            if selected_text:
                ...
                if isinstance(pos, dict):
                    ...
                    if mode == 'multi':
                        ...
                        pos_text = f"Column {col_idx}" if col_idx is not None else "Multi-column"
                    else:
                        percentage = pos.get('percentage', 0) * 100
                        pos_text = f"{percentage:.0f}% through"
                else:
                    pos_text = "Saved position"
            ...
            info_label = Gtk.Label(label=f"{pos_text} • {time_str}")
            ...
            delete_btn.connect("clicked", lambda btn, idx=row.bookmark_index: self._remove_bookmark(idx))
            box.append(delete_btn)
            
            row.set_child(box)
            self.bm_listbox.append(row)
    
    def _on_bookmark_activated(self, listbox, row):
        """Handle bookmark row activation (jump to bookmark)."""
        if hasattr(row, 'bookmark_data'):
            self._jump_to_bookmark(row.bookmark_data)
    ...
    def _save_bookmarks(self):
        ...
        try:
            ...
            self._save_book_settings()
            print(f"[BOOKMARK] 💾 Saved {len(self.bookmarks)} bookmarks")
        except Exception as e:
            print(f"[BOOKMARK] ⚠️ Error saving bookmarks: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_annotation_request(self, manager, js_result):
        """Handle annotation request from JavaScript context menu."""
        try:
            msg = js_result.to_string()
            import json
            data = json.loads(msg)
            ...
            text_before = data.get('before', '')
            text_after = data.get('after', '')
            dom_path = data.get('path', '')
            
            if selected_text:
                self._show_annotation_dialog(selected_text, text_before, text_after, dom_path)
        except Exception as e:
            print(f"[ANNOTATION] Error handling annotation request: {e}")
    ...
    def _show_annotation_dialog(self, selected_text, text_before, text_after, dom_path):
        ...
        text_label.set_markup(f"<b>Selected text:</b>\n{selected_text[:100]}...")
        ...
        for color_name, color_value in self.highlight_colors.items():
            ...
            btn.connect("toggled", lambda b, c=color_name: setattr(self, '_temp_color', c) if b.get_active() else None)
            color_box.append(btn)
        
        content.append(color_box)
        self._temp_color = selected_color
        
        # Note entry
        note_label = Gtk.Label(label="Note (optional):")
        ...
        dialog.connect("response", lambda d, r: self._on_annotation_dialog_response(
            d, r, selected_text, text_before, text_after, dom_path, note_view.get_buffer()
        ))
        
        dialog.present()
    
    def _on_annotation_dialog_response(self, dialog, response, selected_text, text_before, text_after, dom_path, note_buffer):
        """Handle annotation dialog response."""
        if response == Gtk.ResponseType.OK:
            # Get note text
            start_iter = note_buffer.get_start_iter()
            end_iter = note_buffer.get_end_iter()
            note = note_buffer.get_text(start_iter, end_iter, True)
            ...
            annotation = {
                ...
                'dom_path': dom_path,
                'highlight_color': self._temp_color,
                'note': note,
                'timestamp': time.time()
            }
    ...
    def _apply_annotation_highlight(self, annotation):
        """Apply highlight to annotated text in WebView."""
        if not self.webview:
            return
    ...
    def _update_annotations_list(self):
        ...
        # Clear existing
        while True:
            child = self.ann_listbox.get_first_child()
            if child is None:
                break
        ...
        if not self.annotations:
            ...
            label.set_margin_top(20)
            label.set_margin_bottom(20)
            self.ann_listbox.append(label)
            return
        
        # Sort by timestamp (newest first)
        sorted_annotations = sorted(self.annotations, key=lambda a: a['timestamp'], reverse=True)
        
        for annotation in sorted_annotations:
            row = Gtk.ListBoxRow()
            row.annotation_data = annotation
            ...
            color = self.highlight_colors.get(annotation['highlight_color'], '#ffeb3b')
            css = f"box {{ background: {color}; border-radius: 3px; }}"
            css_provider = Gtk.CssProvider()
            css_provider.load_from_data(css.encode())
            ...
            # Chapter title
            chapter_label = Gtk.Label(label=annotation['chapter_title'])
            chapter_label.set_xalign(0)
            ...
            text = annotation['selected_text'][:50] + "..." if len(annotation['selected_text']) > 50 else annotation['selected_text']
            ...
            # Note (if present)
            if annotation['note']:
                note_label = Gtk.Label(label=annotation['note'][:50] + "..." if len(annotation['note']) > 50 else annotation['note'])
                note_label.set_xalign(0)
            ...
            delete_btn.add_css_class("flat")
            delete_btn.connect("clicked", lambda btn, ann_id=annotation['id']: self._remove_annotation(ann_id))
            box.append(delete_btn)
            
            row.set_child(box)
            self.ann_listbox.append(row)
    
    def _on_annotation_activated(self, listbox, row):
        """Handle annotation row activation."""
        if hasattr(row, 'annotation_data'):
            self._jump_to_annotation(row.annotation_data)
    
    def _jump_to_annotation(self, annotation):
    ...
    def _reapply_annotation_highlights(self):
        """Re-apply annotation highlights for the current chapter after page load."""
        if not self.webview or self.current_index is None:
            return
        ...
        try:
            ...
            current_annotations = [
                ...
                if a.get('chapter_index') == self.current_index
            ]
            
            if current_annotations:
                print(f"[ANNOTATION] Re-applying {len(current_annotations)} highlights for chapter {self.current_index}")
    ...
    def _on_define_request(self, manager, js_result):
        """Handle define request from JavaScript context menu."""
        try:
            msg = js_result.to_string()
            import json
            data = json.loads(msg)
            ...
            if word:
                # Set the word in the dictionary entry
            ...
            print(f"[DEFINE] Error handling define request: {e}")
    
    def _lookup_dictionary_word(self):
        """Lookup word in dictionary and display in sidebar."""
        word = self.dict_entry.get_text().strip()
        if not word:
            return
        ...
        def fetch_definition():
            try:
                ...
                req = urllib.request.Request(url, headers={'User-Agent': 'EPUBViewer/1.0'})
                with urllib.request.urlopen(req, timeout=10) as response:
                    import json
                    data = json.loads(response.read().decode('utf-8'))
                    
                    # Build HTML with formatting preserved
                    html_content = self._build_definition_html(word, data)
                    
                    # Update UI in main thread
                    GLib.idle_add(lambda: (self._set_dict_html(html_content), False)[1])
    ...
    def _build_definition_html(self, word, data):
        ...
        if 'en' in data and data['en']:
            for entry in data['en']:
                ...
                html_parts.append(f"<h3>{pos.upper()}:</h3>")
                
                definitions = entry.get('definitions', [])
                for i, definition in enumerate(definitions, 1):
                    def_text = definition.get('definition', '')
                    # Keep HTML formatting but clean up some tags
                    def_text = def_text.replace('</b>', '</b> ')
                    
                    html_parts.append(f'<div class="definition">')
                    html_parts.append(f'<span class="definition-number">{i}.</span> {def_text}')
                    
                    # Add examples if available
                    examples = definition.get('examples', [])
                    for example in examples[:2]:  # Show max 2 examples
                        html_parts.append(f'<div class="example">Example: {example}</div>')
                    
                    html_parts.append('</div>')
        else:
            html_parts.append('<div class="no-definition">No definition found for this word.</div>')
    ...
    def _perform_search(self):
        ...
        if not query:
            self.search_info_label.set_text("Please enter a search term")
            return
        ...
        def search_thread():
                    ...
                    try:
                        ...
                        if query_normalized in text_normalized:
                            ...
                            while True:
                                ...
                                # Get chapter title
                                chapter_title = self._get_chapter_title(idx)
                        ...
                        print(f"[SEARCH] Error searching item {idx}: {e}")
                        continue
            ...
            # Update UI in main thread
            GLib.idle_add(lambda: self._display_search_results(results, query))
    ...
    def _display_search_results(self, results, query):
        """Display search results in the sidebar list."""
        # Clear list
        while True:
            row = self.search_listbox.get_row_at_index(0)
            if row:
        ...
        if not results:
            self.search_info_label.set_text(f"No results found for '{query}'")
            label = Gtk.Label(label="No results found")
            label.set_margin_top(20)
            label.set_margin_bottom(20)
            label.add_css_class("dim-label")
            self.search_listbox.append(label)
            return
        
        self.search_info_label.set_text(f"Found {len(results)} result(s)")
        print(f"[SEARCH] Found {len(results)} results")
        
        for result in results:
            row = Gtk.ListBoxRow()
            row.search_data = result
            ...
            # Chapter title
            title_label = Gtk.Label(label=result['chapter_title'])
            title_label.set_xalign(0)
            title_label.add_css_class("heading")
            title_label.set_ellipsize(Pango.EllipsizeMode.END)
            box.append(title_label)
            
            # Context with highlighted query
            context = result['context']
            query = result['query']
            
            # Create label with markup to highlight the query
            query_lower = query.lower()
            context_lower = context.lower()
            
            # Find query position in context for highlighting
            query_pos = context_lower.find(query_lower)
            if query_pos >= 0:
                before = context[:query_pos]
                match = context[query_pos:query_pos + len(query)]
                after = context[query_pos + len(query):]
                
                # Escape for markup
                import html
                before_escaped = html.escape(before)
                match_escaped = html.escape(match)
                after_escaped = html.escape(after)
                
                markup = f'{before_escaped}<span background="#ffff00" foreground="#000000"><b>{match_escaped}</b></span>{after_escaped}'
            else:
                import html
                markup = html.escape(context)
            
            context_label = Gtk.Label()
            context_label.set_markup(markup)
            context_label.set_xalign(0)
            context_label.set_wrap(True)
            context_label.set_wrap_mode(Pango.WrapMode.WORD_CHAR)
    ...
    def _on_search_result_activated(self, listbox, row):
        """Handle search result click - navigate to that location."""
        if not hasattr(row, 'search_data'):
            return
        
        result = row.search_data
        chapter_index = result['chapter_index']
        query = result['query']
        
        print(f"[SEARCH] Navigating to chapter {chapter_index}, searching for '{query}'")
        ...
        # Highlight the search term in the webview
        def highlight_search():
            try:
                self._highlight_search_term(query)
            except Exception as e:
                print(f"[SEARCH] Error highlighting: {e}")
            return False
        
        # Delay to let page load
        GLib.timeout_add(300, highlight_search)
    
    def _highlight_search_term(self, query):
        """Highlight all occurrences of search term in the current page."""
        if not self.webview or not query:
            return
        
        import json
        query_escaped = json.dumps(query)
        ...
        (function() {{
            try {{
                ...
                // Add search highlight style if not present
                if (!document.querySelector('style[data-search-style]')) {{
                    var style = document.createElement('style');
                    style.dataset.searchStyle = '1';
                    style.textContent = '.search-highlight {{ background-color: #ffff00; color: #000; font-weight: bold; padding: 2px 0; }}';
                    document.head.appendChild(style);
                }}
                ...
                console.log('[SEARCH] Found', positions.length, 'occurrences');
                ...
                for (var i = 0; i < positions.length; i++) {{
                    ...
                    // Find nodes for this occurrence
                    for (var j = 0; j < nodeMap.length; j++) {{
                        ...
                        if (nm.start < idx + query.length && nm.end >= idx + query.length) {{
                            endNode = nm.node;
                            endOffset = (idx + query.length) - nm.start;
                            break;
                        }}
                    }}
                    ...
                    if (startNode && endNode) {{
                        try {{
                            ...
                            console.log('[SEARCH] Error highlighting:', e);
                        }}
                    }}
                }}
                ...
                console.error('[SEARCH] Error in highlight function:', e);
            }}
        }})();
        """
    ...
    def _on_speak_request(self, manager, js_result):
        """Handle speak request from JavaScript context menu."""
        try:
            msg = js_result.to_string()
            import json
            data = json.loads(msg)
            ...
            if text_content and self.tts:
                print(f"[TTS] Speak from here requested, text length: {len(text_content)}")
        ...
        for e in self.library:
            if e.get("path") == self.book_path:
                ...
                for k, v in self.saved_scroll_positions.items():
                    if isinstance(v, dict):
                        # New format: already a dict, just store it
                        scroll_positions_serializable[str(k)] = v
                    elif isinstance(v, (tuple, list)):
                        # Old format: convert tuple to dict for backward compatibility
                        scroll_positions_serializable[str(k)] = {
                            'mode': 'multi' if len(v) > 0 and v[0] > 0 else 'single',
                            'scrollLeft': v[0] if len(v) > 0 else 0,
                            'scrollTop': v[1] if len(v) > 1 else 0,
                            'columnIndex': None,
                            'percentage': 0.0
                        }
