...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # Horizontal scroll position tracking for multi-column layout
        self.saved_scroll_positions = {}  # {chapter_index: scroll_left}
                ...
                def _on_load_changed(webview, load_event):
                    ...
                    try:
                        ...
                        if finished:
                            ...
                            # Restore horizontal scroll position for this chapter
                            GLib.timeout_add(250, lambda: (self._restore_scroll_for_chapter(), False)[1])
          ...
          # set title and app name
            ...
            if resume:
                ...
                if loaded_settings and "scroll_positions" in loaded_settings:
                    self.saved_scroll_positions = dict(loaded_settings.get("scroll_positions", {}))
                else:
                    for e in self.library:
                        if e.get("path") == path:
                            self.saved_scroll_positions = dict(e.get("scroll_positions", {}))
                            break
            else:
...
, "r", encoding="utf-8", errors="ignore") as fh:
    ...
    def cleanup(self):
        # Capture scroll before cleanup
        if hasattr(self, 'book_path') and self.book_path:
            self._capture_current_scroll()
            try:
                GLib.timeout_add(100, lambda: (self._save_progress_for_library(), False)[1])
            except Exception:
                pass
    ...
    def _save_book_settings(self):
        ...
        settings = {
            ...
            "scroll_positions": dict(self.saved_scroll_positions),
        }
    ...
    def _capture_current_scroll(self):
        """Capture the current horizontal scroll position before navigation."""
        ...
        (function() {
            var container = document.querySelector('.ebook-content');
            return container ? (container.scrollLeft || 0) : 0;
        })();
        """
        ...
        def on_result(webview, result, user_data):
            try:
                ...
                if val:
                    scroll_pos = 0
                    if hasattr(val, 'to_double'):
                        scroll_pos = int(val.to_double())
                    elif hasattr(val, 'to_int32'):
                        scroll_pos = val.to_int32()
                    
                    if scroll_pos > 0:
                        self.saved_scroll_positions[self.current_index] = scroll_pos
            except Exception:
                pass
    ...
    def _restore_scroll_for_chapter(self):
        """Restore horizontal scroll position for the current chapter after page load."""
        if not getattr(self, "webview", None) or self.current_index is None:
            return
        
        scroll_pos = self.saved_scroll_positions.get(self.current_index, 0)
        if scroll_pos <= 0:
            return
    ...
    def _save_progress_for_library(self):
        ...
        for e in self.library:
            if e.get("path") == self.book_path:
                ...
                # Save scroll positions dictionary
                e["scroll_positions"] = dict(self.saved_scroll_positions)
                changed = True
                break
