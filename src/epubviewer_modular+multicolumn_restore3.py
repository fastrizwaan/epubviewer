...
class EPubViewer(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        #     'chapter_index': int,
        #     'chapter_title': str,
        #     'position': {mode, scrollLeft, scrollTop, columnIndex, percentage},
        #     'timestamp': float,
        #     'note': str (optional)
        # }
...
, "r", encoding="utf-8", errors="ignore") as fh:
    ...
    def _add_manual_bookmark(self, *args):
        """Add a manual bookmark at current position."""
        print(f"[BOOKMARK] 📑 _add_manual_bookmark called")
        
        if self.current_index is None or not self.book:
            print(f"[BOOKMARK] ⚠️ Cannot add manual bookmark: no book loaded (current_index={self.current_index}, book={bool(self.book)})")
            return
        
        # Get current position
        position = self.saved_scroll_positions.get(self.current_index)
        if not position:
            # Create a position from current state
            position = {
                'mode': 'multi',  # Default assumption
                'scrollLeft': 0,
                'scrollTop': 0,
                'columnIndex': 0,
                'percentage': 0.0
            }
            print(f"[BOOKMARK] ⚠️ No saved position for chapter {self.current_index}, using default")
        else:
            print(f"[BOOKMARK] ✓ Found position for chapter {self.current_index}: {position}")
    ...
    def _add_auto_bookmark(self, chapter_index=None):
        ...
        # Get position for this chapter
        position = self.saved_scroll_positions.get(chapter_index)
        if not position:
            print(f"[BOOKMARK] ⚠️ No position saved for chapter {chapter_index}, skipping auto-bookmark")
            print(f"[BOOKMARK]    Available positions: {list(self.saved_scroll_positions.keys())}")
            return  # Don't auto-save if no position
        
        print(f"[BOOKMARK] ✓ Found position for chapter {chapter_index}: {position}")
    ...
    def _jump_to_bookmark(self, bookmark):
        ...
        try:
            ...
            # Store the position we want to restore to
            self.saved_scroll_positions[chapter_index] = position
            
            # Display the page (which will auto-restore position)
            self.update_navigation()
            self.display_page()
